...
abstract class LekkerCoreListShortcode extends QodeFrameworkShortcode {
	private $post_type;
	...
	public function map_list_options( $params = array() ) {
		...
		$this->set_option( array(
			'field_type'    => 'select',
			'name'          => 'behavior',
			'title'         => esc_html__( 'List Appearance', 'lekker-core' ),
			'options'       => lekker_core_get_select_type_options_pool( 'list_behavior', false, $exclude_behavior ),
		...
		if ( empty( $exclude_behavior ) || ( ! empty( $exclude_behavior ) && ! in_array( 'masonry', $exclude_behavior ) ) ) {
			$this->set_option( array(
				...
				'title'         => esc_html__( 'Image Proportions', 'lekker-core' ),
				'options'       => array(
					''      => esc_html__( 'Original', 'lekker-core' ),
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'images_proportion', $exclude_option ) ) ) {
			$this->set_option( array(
				...
				'default_value' => 'full',
				'title'         => esc_html__( 'Image Proportions', 'lekker-core' ),
				'options'       => lekker_core_get_select_type_options_pool( 'list_image_dimension', false ),
				'group'         => $group,
				'dependency'    => array(
					'show' => array(
						'behavior' => array(
			...
			$this->set_option( array(
				...
				'title'       => esc_html__( 'Custom Image Width', 'lekker-core' ),
				'description' => esc_html__( 'Enter image width in px', 'lekker-core' ),
				'group'       => $group,
				'dependency'  => array(
					'show' => array(
			...
			$this->set_option( array(
				...
				'title'       => esc_html__( 'Custom Image Height', 'lekker-core' ),
				'description' => esc_html__( 'Enter image height in px', 'lekker-core' ),
				'group'       => $group,
				'dependency'  => array(
					'show' => array(
						'images_proportion' => array(
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'columns', $exclude_option ) ) ) {
			$this->set_option( array(
				...
				'options'       => lekker_core_get_select_type_options_pool( 'columns_number', true, $exclude_columns, $include_columns ),
				'default_value' => '3',
				'group'         => $group,
				'dependency'    => array(
					'hide' => array(
			...
			$this->set_option( array(
				...
				'title'         => esc_html__( 'Columns Responsive', 'lekker-core' ),
				'options'       => lekker_core_get_select_type_options_pool( 'columns_responsive' ),
			...
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'columns_1440',
				'title'         => esc_html__( 'Number of Columns 1367px - 1440px', 'lekker-core' ),
			...
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'columns_1366',
				'title'         => esc_html__( 'Number of Columns 1025px - 1366px', 'lekker-core' ),
				'options'       => lekker_core_get_select_type_options_pool( 'columns_number' ),
				'default_value' => '3',
				'dependency'    => array(
					'show' => array(
			...
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'columns_1024',
				'title'         => esc_html__( 'Number of Columns 769px - 1024px', 'lekker-core' ),
				'options'       => lekker_core_get_select_type_options_pool( 'columns_number' ),
				'default_value' => '3',
				'dependency'    => array(
					'show' => array(
			...
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'columns_768',
				'title'         => esc_html__( 'Number of Columns 681px - 768px', 'lekker-core' ),
				'options'       => lekker_core_get_select_type_options_pool( 'columns_number' ),
				'default_value' => '3',
				'dependency'    => array(
					'show' => array(
			...
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'columns_680',
				'title'         => esc_html__( 'Number of Columns 481px - 680px', 'lekker-core' ),
				'options'       => lekker_core_get_select_type_options_pool( 'columns_number' ),
				'default_value' => '3',
				'dependency'    => array(
					'show' => array(
			...
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'columns_480',
				'title'         => esc_html__( 'Number of Columns 0 - 480px', 'lekker-core' ),
				'options'       => lekker_core_get_select_type_options_pool( 'columns_number' ),
				'default_value' => '3',
				'dependency'    => array(
					'show' => array(
		...
		if ( empty( $exclude_behavior ) || ( ! empty( $exclude_behavior ) && ! in_array( 'justified-gallery', $exclude_behavior ) ) ) {
			$this->set_option( array(
				...
				'title'      => esc_html__( 'Row Height', 'lekker-core' ),
				'dependency' => array(
					'show' => array(
						'behavior' => array(
							'values'        => 'justified-gallery',
			...
			$this->set_option( array(
				...
				'title'      => esc_html__( 'Max Row Height', 'lekker-core' ),
				'dependency' => array(
					'show' => array(
						'behavior' => array(
							'values'        => 'justified-gallery',
			...
			$this->set_option( array(
				...
				'title'      => esc_html__( 'Last Row Treshold', 'lekker-core' ),
				'dependency' => array(
					'show' => array(
						'behavior' => array(
							'values'        => 'justified-gallery',
	...
	public function map_slider_options( $params = array() ) {
		...
		$this->set_option( array(
			'field_type' => 'select',
			'name'       => 'slider_loop',
			'title'      => esc_html__( 'Enable Slider Loop', 'lekker-core' ),
		...
		$this->set_option( array(
			...
			'title'      => esc_html__( 'Enable Slider Autoplay', 'lekker-core' ),
			'options'    => lekker_core_get_select_type_options_pool( 'yes_no' ),
			'dependency' => $dependency,
			'group'      => $group
		) );
		$this->set_option( array(
			'field_type'  => 'text',
			'name'        => 'slider_speed',
			'title'       => esc_html__( 'Slide Duration', 'lekker-core' ),
			'description' => esc_html__( 'Default value is 5000 (ms)', 'lekker-core' ),
			'dependency'  => $dependency,
			'group'       => $group
		) );
		$this->set_option( array(
			...
			'title'       => esc_html__( 'Slide Animation Duration', 'lekker-core' ),
			'description' => esc_html__( 'Speed of slide animation in milliseconds. Default value is 800.', 'lekker-core' ),
			'dependency'  => $dependency,
			'group'       => $group
		) );
		$this->set_option( array(
			...
			'title'      => esc_html__( 'Enable Slider Navigation', 'lekker-core' ),
			'options'    => lekker_core_get_select_type_options_pool( 'yes_no' ),
			'dependency' => $dependency,
			'group'      => $group
		) );
		$this->set_option( array(
			...
			'title'      => esc_html__( 'Enable Slider Pagination', 'lekker-core' ),
			'options'    => lekker_core_get_select_type_options_pool( 'yes_no' ),
			'dependency' => $dependency,
			'group'      => $group
		) );
		$this->set_option( array(
			'field_type' => 'select',
			'name'       => 'slider_scroll',
			'title'      => esc_html__( 'Enable Mousewheel Scroll', 'lekker-core' ),
	...
	public function get_list_classes( $atts ) {
		...
		$is_horizontal_slider = ! empty( $atts['custom_sliders'] ) && $atts['custom_sliders'] === 'horizontal-full' ? true : false;

		$holder_classes[] = 'qodef-grid';
		$holder_classes[] = ! empty( $atts['behavior'] ) && $is_horizontal_slider === false && $atts['behavior'] == 'slider' ? 'qodef-swiper-container' : 'qodef-layout--' . $atts['behavior'];
		...
		$holder_classes[] = ! empty( $atts['slider_scroll'] ) && $atts['slider_scroll'] === "yes" ? 'qodef-slider-mousewheel--enabled' : '';
		$holder_classes[] = ! empty( $atts['slider_navigation'] ) && $atts['slider_navigation'] === "no" ?  'qodef-slider-navigation--disabled' : '';
	...
	public function get_list_item_classes( $atts ) {
		...
		$item_classes[] = ! empty( $atts['behavior'] ) && $is_horizontal_slider === false && $atts['behavior'] == 'slider' ? 'swiper-slide' : 'qodef-grid-item';
	...
	public function get_list_item_image_dimension( $atts ) {
		...
		if ( ! empty( $atts['behavior'] ) && $atts['behavior'] == 'masonry' && ! empty( $atts['masonry_images_proportion'] ) && $atts['masonry_images_proportion'] == 'fixed' ) {
			...
			$image_dimension              = lekker_core_get_custom_image_size_meta( 'meta-box', $masonry_image_dimension_name, get_the_ID() );
		}
		
		if ( ! empty( $atts['behavior'] ) && in_array( $atts['behavior'], array( 'columns', 'slider' ) ) ) {
			$image_dimension = array(
				'size'  => $atts['images_proportion'],
				'class' => lekker_core_get_custom_image_size_class_name( $atts['images_proportion'] )
			);
		}
	...
	public function get_slider_data( $atts, $include = array() ) {
		...
		$data['spaceBetween']   = isset( $atts['space'] ) ? lekker_core_get_space_value( $atts['space'] ) : 0;
		...
		$data['sliderScroll'] 	= isset( $atts['slider_scroll'] ) ? $atts['slider_scroll'] != 'no' : '';
		...
		//			$data['spaceBetween'] = 90;
		//		}
		
		if ( isset( $atts['custom_sliders'] ) && $atts['custom_sliders'] == 'vertical-split' ) {
			$data['direction']  = 'vertical';
			$data['sliderScroll'] = true;
		}
		
		if ( isset( $atts['custom_sliders'] ) && $atts['custom_sliders'] == 'horizontal-full' ) {
			$data['sliderScroll'] = true;
		}
	...
	public function map_query_options( $params = array() ) {
		$group                = isset( $params['group'] ) ? $params['group'] : esc_html__( 'Query', 'lekker-core' );
		...
		$this->set_option( array(
			'field_type' => 'text',
			'name'       => 'posts_per_page',
			'title'      => esc_html__( 'Posts per Page', 'lekker-core' ),
			'default'    => '-1',
			'group'      => $group
		) );
		$this->set_option( array(
			'field_type'    => 'select',
			'name'          => 'orderby',
			'title'         => esc_html__( 'Order By', 'lekker-core' ),
			'options'       => lekker_core_get_select_type_options_pool( 'order_by', true, $exclude_order_by, $include_order_by ),
			'default_value' => 'date',
			'group'         => $group
		) );
		$this->set_option( array(
			'field_type'    => 'select',
			'name'          => 'order',
			'title'         => esc_html__( 'Order', 'lekker-core' ),
			'options'       => lekker_core_get_select_type_options_pool( 'order' ),
			'default_value' => 'DESC',
			'group'         => $group
		) );
		
		do_action( 'lekker_core_action_map_query_options_before_additional', $group );
		
		$this->set_option( array(
			'field_type' => 'select',
			'name'       => 'additional_params',
			'title'      => esc_html__( 'Additional Params', 'lekker-core' ),
			'options'    => array(
				''       => esc_html__( 'No', 'lekker-core' ),
				'id'     => esc_html__( 'Post IDs', 'lekker-core' ),
				'tax'    => esc_html__( 'Taxonomy Slug', 'lekker-core' ),
				'author' => esc_html__( 'Author Name', 'lekker-core' ),
			),
			'group'      => $group
		) );
		$this->set_option( array(
			'field_type'  => 'text',
			'name'        => 'post_ids',
			'title'       => esc_html__( 'Posts IDs', 'lekker-core' ),
			'description' => esc_html__( 'Separate post IDs with commas', 'lekker-core' ),
			'group'       => $group,
			'dependency'  => array(
				'show' => array(
		...
		if ( ! empty( $taxonomies_formatted ) ) {
			$this->set_option( array(
				'field_type' => 'select',
				'name'       => 'tax',
				'title'      => esc_html__( 'Taxonomy', 'lekker-core' ),
			...
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'tax_slug',
				'title'      => esc_html__( 'Taxonomy Slug', 'lekker-core' ),
				'group'      => $group,
				'dependency' => array(
					'show' => array(
						'additional_params' => array(
			...
			$this->set_option( array(
				'field_type'  => 'text',
				'name'        => 'tax__in',
				'title'       => esc_html__( 'Taxonomy IDs', 'lekker-core' ),
				'description' => esc_html__( 'Separate taxonomy IDs with commas', 'lekker-core' ),
				'group'       => $group,
				'dependency'  => array(
					'show' => array(
						'additional_params' => array(
		...
		$this->set_option( array(
			'field_type' => 'text',
			'name'       => 'author_slug',
			'title'      => esc_html__( 'Author Slug', 'lekker-core' ),
			'group'      => $group,
			'dependency' => array(
				'show' => array(
					'additional_params' => array(
	...
	public function map_layout_options( $params = array() ) {
		...
		$this->set_option( array(
			'field_type'    => 'select',
			'name'          => 'layout',
			'title'         => esc_html__( 'Item Layout', 'lekker-core' ),
			'options'       => $layouts,
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'text_transform', $exclude_option ) ) ) {
			$this->set_option( array(
				'field_type' => 'select',
				'name'       => 'text_transform',
				'title'      => esc_html__( 'Title Text Transform', 'lekker-core' ),
	...
	public function map_additional_options( $params = array() ) {
		...
		if ( ! $exclude_filter ) {
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'enable_filter',
				'title'         => esc_html__( 'Enable Filter', 'lekker-core' ),
				'description'   => esc_html__( 'Enabling this option will show categories filter above list', 'lekker-core' ),
				...
				'group'         => esc_html__( 'Additional', 'lekker-core' )
			) );
		}
		...
		if ( ! $exclude_pagination ) {
			$this->set_option( array(
				...
				'title'         => esc_html__( 'Pagination', 'lekker-core' ),
				'options'       => lekker_core_get_select_type_options_pool( 'pagination_type' ),
			...
			$this->set_option( array(
				...
				'title'         => esc_html__( 'Load More Top Margin (px or %)', 'lekker-core' ),
				'default_value' => '',
				'group'         => esc_html__( 'Additional', 'lekker-core' ),
				'dependency'    => array(
					'show' => array(
	...
	public function register_list_scripts() {
		...
		$list_scripts = apply_filters( 'lekker_core_filter_register_list_shortcode_scripts', isset( $scripts ) ? $scripts : array() );
		
		$this->set_scripts( $list_scripts );
	}
	
	public function load_assets() {
		do_action( 'lekker_core_action_list_shortcodes_load_assets', $this->get_atts() );
	}
