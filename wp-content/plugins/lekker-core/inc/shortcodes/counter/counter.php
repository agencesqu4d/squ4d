...
if ( ! function_exists( 'lekker_core_add_counter_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function lekker_core_add_counter_shortcode( $shortcodes ) {
		$shortcodes[] = 'LekkerCoreCounterShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'lekker_core_filter_register_shortcodes', 'lekker_core_add_counter_shortcode' );
}

if ( class_exists( 'LekkerCoreShortcode' ) ) {
	class LekkerCoreCounterShortcode extends LekkerCoreShortcode {
		
		public function __construct() {
			$this->set_layouts( apply_filters( 'lekker_core_filter_counter_layouts', array() ) );
			
			parent::__construct();
		}
		
		public function map_shortcode() {
			$this->set_shortcode_path( LEKKER_CORE_SHORTCODES_URL_PATH . '/counter' );
			$this->set_base( 'lekker_core_counter' );
			$this->set_name( esc_html__( 'Counter', 'lekker-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays counter with provided parameters', 'lekker-core' ) );
			...
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'start_digit',
				'title'      => esc_html__( 'Start Digit', 'lekker-core' ),
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'end_digit',
				'title'      => esc_html__( 'End Digit', 'lekker-core' ),
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'step_digit',
				'title'      => esc_html__( 'Step Between Digits', 'lekker-core' ),
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'step_delay',
				'title'      => esc_html__( 'Step Delay', 'lekker-core' ),
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'digit_label',
				'title'      => esc_html__( 'Digit Label', 'lekker-core' ),
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'digit_font_size',
				'title'      => esc_html__( 'Digit Font Size', 'lekker-core' ),
			) );
			$this->set_option( array(
				'field_type' => 'color',
				'name'       => 'digit_color',
				'title'      => esc_html__( 'Digit Color', 'lekker-core' )
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'title',
			...
			$this->set_option( array(
				...
				'title'      => esc_html__( 'Custom Title Font Size', 'lekker-core' ),
				'group'         => esc_html__( 'Content', 'lekker-core' )
			) );
			$this->set_option( array(
				'field_type' => 'color',
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'lekker_core_counter', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return lekker_core_get_template_part( 'shortcodes/counter', 'variations/'.$atts['layout'].'/templates/counter', '', $atts );
		}
