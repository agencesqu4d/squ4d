...
if ( ! function_exists( 'lekker_core_add_banner_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function lekker_core_add_banner_shortcode( $shortcodes ) {
		$shortcodes[] = 'LekkerCoreBannerShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'lekker_core_filter_register_shortcodes', 'lekker_core_add_banner_shortcode' );
}

if ( class_exists( 'LekkerCoreShortcode' ) ) {
	class LekkerCoreBannerShortcode extends LekkerCoreShortcode {
		
		public function __construct() {
			$this->set_layouts( apply_filters( 'lekker_core_filter_banner_layouts', array() ) );
			$this->set_extra_options( apply_filters( 'lekker_core_filter_banner_extra_options', array() ) );
			
			parent::__construct();
		}
		
		public function map_shortcode() {
			$this->set_shortcode_path( LEKKER_CORE_SHORTCODES_URL_PATH . '/banner' );
			$this->set_base( 'lekker_core_banner' );
			$this->set_name( esc_html__( 'Banner', 'lekker-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds banner element', 'lekker-core' ) );
			...
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'subtitle_tag',
				'title'         => esc_html__( 'Subtitle Tag', 'lekker-core' ),
				'options'       => lekker_core_get_select_type_options_pool( 'title_tag' ),
				'default_value' => 'h5',
				'group'         => esc_html__( 'Content', 'lekker-core' )
			) );
			$this->set_option( array(
				'field_type' => 'color',
				'name'       => 'subtitle_color',
				'title'      => esc_html__( 'Subtitle Color', 'lekker-core' ),
				'group'      => esc_html__( 'Content', 'lekker-core' )
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'subtitle_margin_top',
				'title'      => esc_html__( 'Subtitle Margin Top', 'lekker-core' ),
				'group'      => esc_html__( 'Content', 'lekker-core' )
			) );
			$this->set_option( array(
				'field_type' => 'text',
		...
		public function render( $options, $content = null ) {
			...
			return lekker_core_get_template_part( 'shortcodes/banner', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}
