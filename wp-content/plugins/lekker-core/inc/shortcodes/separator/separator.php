...
if ( ! function_exists( 'lekker_core_add_separator_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function lekker_core_add_separator_shortcode( $shortcodes ) {
		$shortcodes[] = 'LekkerCoreSeparatorShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'lekker_core_filter_register_shortcodes', 'lekker_core_add_separator_shortcode', 9 );
}

if ( class_exists( 'LekkerCoreShortcode' ) ) {
	class LekkerCoreSeparatorShortcode extends LekkerCoreShortcode {
		
		public function map_shortcode() {
			$this->set_shortcode_path( LEKKER_CORE_SHORTCODES_URL_PATH . '/separator' );
			$this->set_base( 'lekker_core_separator' );
			$this->set_name( esc_html__( 'Separator', 'lekker-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays separator with provided parameters', 'lekker-core' ) );
			...
			$this->set_option( array(
				'field_type' => 'select',
				'name'       => 'position',
				'title'      => esc_html__( 'Position', 'lekker-core' ),
				'options'    => array(
			...
			$this->set_option( array(
				'field_type' => 'select',
				'name'       => 'border_style',
				'title'      => esc_html__( 'Border Style', 'lekker-core' ),
				'options'    => lekker_core_get_select_type_options_pool( 'border_style' ),
				'group'      => esc_html__( 'Style', 'lekker-core' )
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'width',
				'title'      => esc_html__( 'Width (px or %)', 'lekker-core' ),
				'group'      => esc_html__( 'Style', 'lekker-core' )
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'thickness',
				'title'      => esc_html__( 'Thickness (px)', 'lekker-core' ),
				'group'      => esc_html__( 'Style', 'lekker-core' )
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'margin_top',
				'title'      => esc_html__( 'Margin Top (px or %)', 'lekker-core' ),
				'group'      => esc_html__( 'Style', 'lekker-core' )
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'margin_bottom',
				'title'      => esc_html__( 'Margin Bottom (px or %)', 'lekker-core' ),
				'group'      => esc_html__( 'Style', 'lekker-core' )
			) );
		}
		...
		public function render( $options, $content = null ) {
			...
			return lekker_core_get_template_part( 'shortcodes/separator', 'templates/separator', '', $atts );
		}
