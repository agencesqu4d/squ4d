...
if ( ! function_exists( 'lekker_core_add_highlight_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function lekker_core_add_highlight_shortcode( $shortcodes ) {
		$shortcodes[] = 'LekkerCoreHighlightShortcode';

		return $shortcodes;
	}

	add_filter( 'lekker_core_filter_register_shortcodes', 'lekker_core_add_highlight_shortcode' );
}

if ( class_exists( 'LekkerCoreShortcode' ) ) {
	class LekkerCoreHighlightShortcode extends LekkerCoreShortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( LEKKER_CORE_SHORTCODES_URL_PATH . '/highlight' );
			$this->set_base( 'lekker_core_highlight' );
			$this->set_name( esc_html__( 'Highlight', 'lekker-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays highlight with provided parameters', 'lekker-core' ) );
			...
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'text_tag',
				'title'         => esc_html__( 'Text Tag', 'lekker-core' ),
			...
			$this->set_option( array(
				...
				'title'       => esc_html__( 'Positions of Line Break', 'lekker-core' ),
				'description' => esc_html__( 'Enter the positions of the words after which you would like to create a line break. Separate the positions with commas (e.g. if you would like the first, third, and fourth word to have a line break, you would enter "1,3,4")', 'lekker-core' )
			) );
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'disable_title_break_words',
				'title'         => esc_html__( 'Disable Text Line Break', 'lekker-core' ),
				'description'   => esc_html__( 'Enabling this option will disable text line breaks for screen size 1024 and lower', 'lekker-core' ),
			...
			$this->set_option( array(
				...
				'title'       => esc_html__( 'Highlight Text Position', 'lekker-core' ),
				'description' => esc_html__( 'Enter the positions of the words you would like to display as "highlight" text. Separate start and end positions with commas (e.g. if you would like to wrap from fifth to ninth words, you would enter "5,9") or if you want to highlight whole text fill -1', 'lekker-core' ),
			) );
			$this->set_option( array(
				'field_type' => 'color',
				'name'       => 'highlight_color',
				'title'      => esc_html__( 'Highlight Text Color', 'lekker-core' )
			) );
			$this->set_option( array(
				'field_type' => 'color',
				'name'       => 'highlight_background_color',
				'title'      => esc_html__( 'Highlight Text Background Color', 'lekker-core' )
			) );
			$this->set_option( array(
				'field_type'    => 'text',
				'name'          => 'link',
		...
		public function render( $options, $content = null ) {
			...
			return lekker_core_get_template_part( 'shortcodes/highlight', 'templates/highlight', '', $atts );
		}
