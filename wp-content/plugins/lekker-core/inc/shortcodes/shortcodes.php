...
if ( ! class_exists( 'LekkerCoreShortcodes' ) ) {
	class LekkerCoreShortcodes {
		...
		function init() {
			...
			include_once LEKKER_CORE_SHORTCODES_PATH . '/dashboard/customizer/shortcodes-customizer-options.php';
		}
		...
		function set_enabled_shortcodes() {
			...
			foreach ( glob( LEKKER_CORE_SHORTCODES_PATH . '/*', GLOB_ONLYDIR ) as $shortcode ) {
				...
				if ( basename( $shortcode ) !== 'dashboard' ) {
					$is_disabled = lekker_core_performance_get_option_value( $shortcode, 'lekker_core_performance_shortcode_' );
		...
		function include_shortcode_classes() {
			include_once LEKKER_CORE_SHORTCODES_PATH . '/shortcode.php';
			include_once LEKKER_CORE_SHORTCODES_PATH . '/list-shortcode.php';
		}
		...
		function register_shortcodes() {
			...
			$shortcodes     = apply_filters( 'lekker_core_filter_register_shortcodes', $shortcodes = array() );
	...
	LekkerCoreShortcodes::get_instance();
}