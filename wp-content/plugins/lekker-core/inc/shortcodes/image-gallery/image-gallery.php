...
if ( ! function_exists( 'lekker_core_add_image_gallery_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function lekker_core_add_image_gallery_shortcode( $shortcodes ) {
		$shortcodes[] = 'LekkerCoreImageGalleryShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'lekker_core_filter_register_shortcodes', 'lekker_core_add_image_gallery_shortcode' );
}

if ( class_exists( 'LekkerCoreListShortcode' ) ) {
	class LekkerCoreImageGalleryShortcode extends LekkerCoreListShortcode {
		
		public function map_shortcode() {
			$this->set_shortcode_path( LEKKER_CORE_SHORTCODES_URL_PATH . '/image-gallery' );
			$this->set_base( 'lekker_core_image_gallery' );
			$this->set_name( esc_html__( 'Image Gallery', 'lekker-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds image gallery element', 'lekker-core' ) );
			...
			$this->set_option( array(
				...
				'name'       => 'images',
				'multiple'   => 'yes',
				'title'      => esc_html__( 'Images', 'lekker-core' ),
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'image_size',
			...
			$this->map_list_options( array(
				...
				'group'            => esc_html__( 'Gallery Settings', 'lekker-core' )
			) );
		}

		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'lekker_core_image_gallery', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return lekker_core_get_template_part( 'shortcodes/image-gallery', 'templates/image-gallery', $atts['behavior'], $atts );
		}
