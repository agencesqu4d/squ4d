...
if ( ! function_exists( 'lekker_core_add_image_marquee_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function lekker_core_add_image_marquee_shortcode( $shortcodes ) {
		$shortcodes[] = 'LekkerCoreImageMarqueeShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'lekker_core_filter_register_shortcodes', 'lekker_core_add_image_marquee_shortcode' );
}

if ( class_exists( 'LekkerCoreShortcode' ) ) {
	class LekkerCoreImageMarqueeShortcode extends LekkerCoreShortcode {
		
		public function __construct() {
			$this->set_layouts( apply_filters( 'lekker_core_filter_image_marquee_layouts', array() ) );
			$this->set_extra_options( apply_filters( 'lekker_core_filter_image_marquee_extra_options', array() ) );
			
			parent::__construct();
		}
		
		public function map_shortcode() {
			$this->set_shortcode_path( LEKKER_CORE_SHORTCODES_URL_PATH . '/image-marquee' );
			$this->set_base( 'lekker_core_image_marquee' );
			$this->set_name( esc_html__( 'Image Marquee', 'lekker-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds Image Marquee element', 'lekker-core' ) );
			...
			$this->set_option( array(
				'field_type'    => 'text',
				'name'          => 'duration',
				'title'         => esc_html__( 'Animation Duration (Seconds)', 'lekker-core' ),
				'default_value' => '20',
			) );
			$this->set_option( array(
				'field_type' => 'select',
		...
		public function render( $options, $content = null ) {
			...
			return lekker_core_get_template_part( 'shortcodes/image-marquee', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}
