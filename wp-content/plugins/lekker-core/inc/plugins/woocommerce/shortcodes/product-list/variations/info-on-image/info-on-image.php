...
if ( ! function_exists( 'lekker_core_add_product_list_variation_info_on_image' ) ) {
	function lekker_core_add_product_list_variation_info_on_image( $variations ) {
		$variations['info-on-image'] = esc_html__( 'Info On Image', 'lekker-core' );
		
		return $variations;
	}
	
	add_filter( 'lekker_core_filter_product_list_layouts', 'lekker_core_add_product_list_variation_info_on_image' );
}

if ( ! function_exists( 'lekker_core_register_shop_list_info_on_image_actions' ) ) {
	function lekker_core_register_shop_list_info_on_image_actions() {
		...
		remove_action( 'woocommerce_shop_loop_item_title', 'lekker_woo_shop_loop_item_title', 10 ); // permission 10 is default
		add_action( 'woocommerce_before_shop_loop_item_title', 'lekker_woo_shop_loop_item_title', 17 ); // permission 17 is set because lekker_add_product_list_item_additional_image_holder hook is added on 15
		...
		add_action( 'woocommerce_before_shop_loop_item_title', 'woocommerce_template_loop_price', 19 ); // permission 19 is set because lekker_woo_shop_loop_item_title hook is added on 17
	...
	add_action( 'lekker_core_action_shop_list_item_layout_info-on-image', 'lekker_core_register_shop_list_info_on_image_actions' );
}