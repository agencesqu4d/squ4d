...
class LekkerCoreTitles {
	...
	public function __construct() {
		...
		add_filter( 'lekker_filter_page_title_classes', array( $this, 'add_page_title_classes' ) );

		// Add title area inline styles
		add_filter( 'lekker_filter_add_inline_style', array( $this, 'add_inline_styles' ) );
	}
	...
	function include_layouts() {
		...
		foreach ( glob( LEKKER_CORE_INC_PATH . '/title/dashboard/*/*.php' ) as $admin ) {
			include_once $admin;
		}

		foreach ( glob( LEKKER_CORE_INC_PATH . '/title/layouts/*/include.php' ) as $layout ) {
			include_once $layout;
		}
	}

	function set_variables() {
		$layout_meta = lekker_core_get_post_value_through_levels( 'qodef_title_layout' );
		$layouts     = apply_filters( 'lekker_core_filter_register_title_layouts', $layouts = array() );
	...
	function render_template() {
		...
		if ( ! empty( $title_object ) ) {
			$template_hook = $title_object->overriding_whole_title ? 'lekker_filter_title_template' : 'lekker_filter_title_content_template';
	...
	function add_page_title_classes( $classes ) {
		$layout             = lekker_core_get_post_value_through_levels( 'qodef_title_layout' );
		$text_alignment     = lekker_core_get_post_value_through_levels( 'qodef_page_title_text_alignment' );
		$vertical_alignment = lekker_core_get_post_value_through_levels( 'qodef_page_title_vertical_text_alignment' );
		$image_params       = lekker_core_get_page_title_image_params();
	...
	function add_inline_styles( $style ) {
		...
		$height           	  	  = lekker_core_get_post_value_through_levels( 'qodef_page_title_height' );
		$height_mobile     	  	  = lekker_core_get_post_value_through_levels( 'qodef_page_title_height_on_smaller_screens' );
		$title_padding    	  	  = apply_filters( 'lekker_core_filter_title_padding', 0 );
		$title_padding_mobile 	  = apply_filters( 'lekker_core_filter_title_padding_mobile', 0 );
		$title_vertical_alignment = lekker_core_get_post_value_through_levels( 'qodef_page_title_vertical_text_alignment' );
		$background_color 	  	  = lekker_core_get_post_value_through_levels( 'qodef_page_title_background_color' );
		$image_params     	  	  = lekker_core_get_page_title_image_params();
		...
		$title_color = lekker_core_get_post_value_through_levels( 'qodef_page_title_color' );
