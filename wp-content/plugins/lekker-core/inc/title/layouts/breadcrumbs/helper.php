...
if ( ! function_exists( 'lekker_core_register_breadcrumbs_title_layout' ) ) {
	function lekker_core_register_breadcrumbs_title_layout( $layouts ) {
		$layouts['breadcrumbs'] = 'LekkerCoreBreadcrumbsTitle';
		
		return $layouts;
	}
	
	add_filter( 'lekker_core_filter_register_title_layouts', 'lekker_core_register_breadcrumbs_title_layout');
}

if ( ! function_exists( 'lekker_core_add_breadcrumbs_title_layout_option' ) ) {
	/**
	 ...
	 * @return array
	 */
	function lekker_core_add_breadcrumbs_title_layout_option( $layouts ) {
		$layouts['breadcrumbs'] = esc_html__( 'Breadcrumbs', 'lekker-core' );
		
		return $layouts;
	}
	
	add_filter( 'lekker_core_filter_title_layout_options', 'lekker_core_add_breadcrumbs_title_layout_option' );
}

if ( ! function_exists( 'lekker_core_breadcrumbs' ) ) {
	/**
	 * Function that renders breadcrumbs html
	 */
	function lekker_core_breadcrumbs() {
		$page_id = qode_framework_get_page_id();
		
		// Breadcrumbs label
		$labels = apply_filters( 'lekker_core_filter_breadcrumbs_label', array(
			'home'        => esc_html__( 'Home', 'lekker-core' ),
			'tag'         => esc_html__( 'Posts tagged "%s"', 'lekker-core' ),
			'author'      => esc_html__( 'Posted by %s', 'lekker-core' ),
			'search'      => esc_html__( 'Search results for "%s"', 'lekker-core' ),
			'404'         => esc_html__( '404 - Page not found', 'lekker-core' ),
			'query_paged' => esc_html__( '(Page %s)', 'lekker-core' )
		) );
		
		// Breadcrumbs variables
		$settings = apply_filters( 'lekker_core_filter_breadcrumbs_settings', array(
		...
		$breadcrumbs_html = '';
		if ( ! empty( $wrap ) ) {
			$breadcrumbs_html = $settings['wrap_before'] . $wrap . apply_filters( 'lekker_core_filter_breadcrumbs_content', $wrap_child, $settings ) . $settings['wrap_after'];
		}
		
		echo apply_filters( 'lekker_core_filter_breadcrumbs_template', $breadcrumbs_html );
	}
}