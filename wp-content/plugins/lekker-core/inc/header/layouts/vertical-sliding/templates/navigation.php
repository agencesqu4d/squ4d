<?php if ( has_nav_menu( 'vertical-menu-navigation' ) || has_nav_menu( 'main-navigation' ) ) { ?>
    <nav class="qodef-header-vertical-sliding-navigation qodef-vertical-sliding-drop-down--below" role="navigation" aria-label="<?php esc_attr_e( 'Vertical Sliding Menu', 'lekker-core' ); ?>">
		<?php
		...
		wp_nav_menu( array(
			...
			'link_after'     => '</span>',
			'walker'         => new LekkerCoreRootMainMenuWalker()
		) );
		?>
    </nav>
<?php } ?>