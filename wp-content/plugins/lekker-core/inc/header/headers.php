...
class LekkerCoreHeaders {
	...
	public function __construct() {
		...
		add_action( 'lekker_action_after_page_header_inner', array( $this, 'scroll_appearance' ) );
	...
	function include_elements() {
		...
		foreach ( glob( LEKKER_CORE_INC_PATH . '/header/dashboard/*/*.php' ) as $admin ) {
			include_once $admin;
		}
		
		foreach ( glob( LEKKER_CORE_INC_PATH . '/header/layouts/*/include.php' ) as $layout ) {
			include_once $layout;
		}
		
		foreach ( glob( LEKKER_CORE_INC_PATH . '/header/*/include.php' ) as $header_part ) {
			include_once $header_part;
		}
	}

	function set_variables() {
		$layout_meta = lekker_core_get_post_value_through_levels( 'qodef_header_layout' );
		$layouts     = apply_filters( 'lekker_core_filter_register_header_layouts', $header_layouts_option = array() );
	...
	function render_template() {
		...
		if ( ! empty( $header_object ) ) {
			$template_hook = $header_object->overriding_whole_header ? 'lekker_filter_header_template' : 'lekker_filter_header_content_template';
	...
	function add_body_classes( $classes ) {
		$header_layout            = lekker_core_get_post_value_through_levels( 'qodef_header_layout' );
		$header_skin              = lekker_core_get_post_value_through_levels( 'qodef_header_skin' );
	...
	function scroll_appearance() {
		...
		if ( ! empty( $header_object ) ) {
			$appearance_type = lekker_core_get_post_value_through_levels( 'qodef_header_scroll_appearance' );

			if ( file_exists( LEKKER_CORE_HEADER_LAYOUTS_PATH . '/' . $header_object->slug . '/templates/' . $appearance_type . '.php' ) ) {
				$scroll_appearance_layout = 'layouts/' . $header_object->slug;
			} else {
				$scroll_appearance_layout = 'scroll-appearance/' . $appearance_type;
			}

			lekker_core_template_part( 'header/' . $scroll_appearance_layout, 'templates/' . $appearance_type, '', array() );
		}
	}
	...
	function add_header_widget_areas() {
		register_sidebar(
			array(
				...
				'name'          => esc_html__( 'Header - Area One', 'lekker-core' ),
				'description'   => esc_html__( 'Widgets added here will appear in header widget area one', 'lekker-core' ),
		...
		register_sidebar(
			array(
				...
				'name'          => esc_html__( 'Header - Area Two', 'lekker-core' ),
				'description'   => esc_html__( 'Widgets added here will appear in header widget area two', 'lekker-core' ),
		...
		do_action( 'lekker_core_action_additional_header_widgets_area' );
	}
}

LekkerCoreHeaders::get_instance();