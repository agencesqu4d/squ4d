...
if ( ! function_exists( 'lekker_core_add_page_sidebar_meta_box' ) ) {
	/**
	 * Function that add general options for this module
	 */
	function lekker_core_add_page_sidebar_meta_box( $page ) {
		...
		if ( $page ) {
			...
			$sidebar_tab = $page->add_tab_element(
				array(
					...
					'description' => esc_html__( 'Sidebar layout settings', 'lekker-core' )
				)
			);
			...
			$sidebar_tab->add_field_element(
				array(
					...
					'description'   => esc_html__( 'Choose a sidebar layout', 'lekker-core' ),
			...
			if ( ! empty( $custom_sidebars ) && count( $custom_sidebars ) > 1 ) {
				$sidebar_tab->add_field_element(
					array(
						...
						'description' => esc_html__( 'Choose a custom sidebar', 'lekker-core' ),
			...
			do_action( 'lekker_core_action_after_page_sidebar_meta_box_map', $sidebar_tab );
		}
	}
	
	add_action( 'lekker_core_action_after_general_meta_box_map', 'lekker_core_add_page_sidebar_meta_box' );
}