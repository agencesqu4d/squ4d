...
if ( ! function_exists( 'lekker_include_comments_in_templates' ) ) {
	/**
	 * Function which includes comments templates on pages/posts
	 */
	function lekker_include_comments_in_templates() {
		
		// Include comments template
		comments_template();
	}
	
	add_action( 'lekker_action_after_page_content', 'lekker_include_comments_in_templates', 100 ); // permission 100 is set to comments template be at the last place
	add_action( 'lekker_action_after_blog_post_item', 'lekker_include_comments_in_templates', 100 );
}

if ( ! function_exists( 'lekker_is_page_comments_enabled' ) ) {
	/**
	 * Function that check is module enabled
	 */
	function lekker_is_page_comments_enabled() {
		$is_enabled = apply_filters( 'lekker_filter_enable_page_comments', true );
		
		return $is_enabled;
	}
}

if ( ! function_exists( 'lekker_load_page_comments' ) ) {
	/**
	 * Function which loads page template module
	 */
	function lekker_load_page_comments() {
		
		if ( lekker_is_page_comments_enabled() ) {
			lekker_template_part( 'comments', 'templates/comments' );
		}
	}
	
	add_action( 'lekker_action_page_comments_template', 'lekker_load_page_comments' );
}

if ( ! function_exists( 'lekker_get_comments_list_template' ) ) {
	/**
	 ...
	 */
	function lekker_get_comments_list_template( $comment, $args, $depth ) {
		global $post;
			...
			<div class="qodef-e-content">
				...
				<div class="qodef-e-links">
					...
					comment_reply_link( array_merge( $args, array(
						'reply_text' => esc_html__( 'Reply', 'lekker' ),
						'depth'      => $depth,
						'max_depth'  => $args['max_depth']
					) ) );
					
					edit_comment_link( esc_html__( 'Edit', 'lekker' ) ); ?>
				</div>
			</div>
		</div>
