...
if ( ! class_exists( 'LekkerWelcomePage' ) ) {
	class LekkerWelcomePage {
		/**
		 * singleton class
		 */
		...
		/**
		 * get the instance of LekkerWelcomePage
		 *
		 * @return self
		 */
		...
		function init_activation_hook() {
			if ( ! is_network_admin() ) {
				set_transient( '_lekker_welcome_page_redirect', 1, 30 );
			}
		}
		...
		function welcome_page_redirect() {
			...
			if ( ! get_transient( '_lekker_welcome_page_redirect' ) ) {
				return;
			}

			// delete the redirect transient
			delete_transient( '_lekker_welcome_page_redirect' );
			...
			wp_safe_redirect( add_query_arg( array( 'page' => 'lekker_welcome_page' ), esc_url( admin_url( 'themes.php' ) ) ) );
			exit;
		}
		...
		function create_welcome_page() {
			...
			remove_submenu_page( 'themes.php', 'lekker_welcome_page' );
		}
		...
		function welcome_page_content() {
			...
			$params['theme_screenshot']  = file_exists( LEKKER_ROOT_DIR . '/screenshot.png' ) ? LEKKER_ROOT . '/screenshot.png' : LEKKER_ROOT . '/screenshot.jpg';

			lekker_template_part( 'welcome', 'templates/welcome', '', $params );
		}
		...
		function enqueue_styles( $hook ) {
			if ( $hook === 'appearance_page_lekker_welcome_page' ) {
				wp_enqueue_style( 'lekker-welcome-page-style', LEKKER_INC_ROOT . '/welcome/assets/admin/css/welcome.min.css' );
			}
		}
	}
}

LekkerWelcomePage::get_instance();